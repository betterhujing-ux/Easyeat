...
export const Navigation: React.FC<NavigationProps> = ({ currentTab, onTabChange }) => {
  return (
    <nav
      ...
      role="tablist"
      aria-label="主要导航"
      className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-neutral-200/80 max-w-xl mx-auto px-12 pt-2.5 pb-[max(1rem,env(safe-area-inset-bottom))] flex justify-around items-center transition-colors"
    >
      {/* 菜谱库 */}
      <button
        id="nav-tab-recipes"
      ...
      >
        ...
        <span className="text-[11px] tracking-tight">饮食记录</span>
      </button>
    </nav>
  );
};
